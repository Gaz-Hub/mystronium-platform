...
import { Wand2, BookOpen, Sparkles, Save, Edit, Trash2, Plus } from 'lucide-react';
...
const Ghostscribe = () => {
  ...
  const generateText = async () => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        toast.error('API key invalid. Please check your Mistral API configuration.');
  ...
  const handleUpdate = async () => {
    ...
    try {
      await updateDoc(doc(db, 'books', currentUser.uid, 'chapters', selectedId), {
        title: title.trim(),
        content: output,
      });
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {userProfile?.subscription === 'free' && (
              <div className="bg-yellow-900/20 border border-yellow-500 text-yellow-400 p-4 rounded-lg">
                ...
                <p className="text-sm">Upgrade to Premium for GPT-4o access and unlimited generations</p>
              </div>
            )}
          </div>
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {chapters.length > 0 ? (
                chapters.map((chapter) => (
                  <div key={chapter.id} className="p-3 bg-gray-700/50 rounded-lg border border-gray-600">
                    <div className="flex justify-between items-start mb-2">
                      <strong className="text-white text-sm">{chapter.title}</strong>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500 italic text-sm">No chapters yet</p>
                  <p className="text-gray-600 text-xs">Generate and save your first chapter</p>
                </div>
              )}
            </div>
          </div>
