...
import { BookOpen, Star, Search, Filter, ShoppingCart, Crown, Play } from 'lucide-react';
...
const Marketplace = () => {
  ...
  const marketplaceItems: MarketplaceItem[] = [
    {
      ...
      rating: 4.5,
      genre: 'fantasy',
      description: 'A mystical journey through realms unknown, where magic and technology collide.',
      sales: 234,
      type: 'book',
      featured: true
    },
    {
      ...
      rating: 4.8,
      genre: 'sci-fi',
      description: 'Visual storytelling at its finest - a cyberpunk adventure in comic form.',
      sales: 156,
      type: 'comic',
      premium: true
    },
  ...
  const genres = ['all', 'fantasy', 'sci-fi', 'mystery', 'romance', 'thriller', 'horror', 'anthology'];
  ...
  const filteredItems = marketplaceItems.filter(item => {
    ...
    if (priceFilter === 'free') matchesPrice = item.free || item.price === 'Free';
    else if (priceFilter === 'under-5') matchesPrice = parseFloat(item.price.replace('£', '')) < 5;
    else if (priceFilter === 'under-10') matchesPrice = parseFloat(item.price.replace('£', '')) < 10;
    else if (priceFilter === 'premium') matchesPrice = item.premium || parseFloat(item.price.replace('£', '')) >= 10;
  ...
  const handlePurchase = (item: MarketplaceItem) => {
    if (item.free) {
      ...
      toast.success(`Added "${item.title}" to cart! Redirecting to checkout...`);
    }
  };
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'comic': return <span className="text-sm">🎬</span>;
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'comic': return 'text-pink-400';
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        >
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400">Discover books, comics, and audio content from our creator community</p>
          </div>
                ...
                {priceFilters.map(filter => (
                  <option key={filter} value={filter}>
                    {filter === 'all' ? 'All Prices' : 
                     ...
                     filter === 'under-5' ? 'Under £5' :
                     filter === 'under-10' ? 'Under £10' :
                     'Premium (£10+)'}
                  </option>
                ))}
              </select>
          ...
          >
            <h3 className="text-2xl font-bold text-white mb-4">Ready to Publish Your Content?</h3>
            <p className="text-gray-300 mb-6">
              Join our community of creators and start earning from your books, comics, and audio content
            </p>
            <Link
              to="/creator-backend"
