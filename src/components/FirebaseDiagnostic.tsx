...
import { runFullDiagnostic, logDiagnosticResult, DiagnosticResult } from '../utils/diagnostic';
  ...
  const testFirebaseConnection = (): Unsubscribe | undefined => {
    ...
    try {
      ...
      if (auth && db) {
        ...
        logDiagnostic('🛠️ Running in demo mode - check .env.local configuration');
        logDiagnostic('📝 Configure Firebase project at: https://console.firebase.google.com/');
        return;
      }
      ...
      if (missingVars.length > 0) {
        logDiagnostic(`⚠️ Missing environment variables: ${missingVars.join(', ')}`);
  ...
  const testUserProfile = async (user: User) => {
    try {
      ...
      if (userSnap.exists()) {
        ...
        logDiagnostic(`✅ User profile exists: ${JSON.stringify(userData, null, 2)}`);
      ...
      if (error.code === 'permission-denied') {
        ...
        logDiagnostic('📝 Deploy rules: firebase deploy --only firestore:rules');
      }
    }
  };
      ...
      {diagnosticResult && (
        <div className="space-y-2 mb-4">
          ...
          <div className="text-xs space-y-1">
            <div>Config: {diagnosticResult.firebaseConfig.hasValidConfig ? '✅' : '❌'}</div>
            <div>Auth: {diagnosticResult.firebaseServices.auth ? '✅' : '❌'}</div>
            <div>Firestore: {diagnosticResult.firebaseServices.db ? '✅' : '❌'}</div>
            <div>User: {diagnosticResult.authentication.isAuthenticated ? '✅' : '❌'}</div>
            <div>Admin: {diagnosticResult.authentication.isAdmin ? '👑' : '👤'}</div>
        ...
        <div className="text-xs bg-gray-800 p-2 rounded">
          {currentUser ? (
            <>
              ...
              <div>Admin: {currentUser.email === 'garetharjohns@gmail.com' ? 'Yes' : 'No'}</div>
            </>
          ) : (
            'Not authenticated'
          )}
      ...
      <div className="mt-4">
        ...
        <div className="bg-black p-2 rounded text-xs max-h-32 overflow-y-auto">
          {diagnostics.length === 0 ? (
            <div className="text-gray-500">No logs yet. Run diagnostic to see results.</div>
          ) : (
