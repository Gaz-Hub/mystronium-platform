...
export const useCreditProtection = () => {
  ...
  useEffect(() => {
    ...
    if (userProfile?.subscription === 'free') {
      ...
      if (usageRatio >= config.emergencyThreshold) {
        ...
        toast.error('🚨 Daily limit reached! Upgrade to continue using AI tools.');
      } else if (usageRatio >= config.warningThreshold) {
        setProtectionActive(true);
        toast(`⚠️ You've used ${Math.round(usageRatio * 100)}% of your daily limit.`, {
          icon: '⚠️',
          style: {
            background: '#f59e0b',
            color: 'white',
  ...
  const getUsagePercentage = (): number => {
    ...
    return Math.min(100, (usageStats.dailyApiCalls / config.dailyFreeLimit) * 100);
  };
  ...
  const forceDisableHighCostFeatures = () => {
    ...
    setEmergencyMode(true);
    toast.error('🚨 High-cost features temporarily disabled due to usage limits.');
  };
