...
export function useUsageCheck() {
  ...
  const incrementUsage = async () => {
    ...
    if (userProfile && userProfile.subscription === 'free' && userProfile.vaultCredits > 0) {
  ...
  return { 
    ...
    incrementUsage, 
    usage: godModeEnabled ? Infinity : (userProfile?.subscription === 'free' ? userProfile.vaultCredits : Infinity), 
