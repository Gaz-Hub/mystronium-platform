...
export const runFullDiagnostic = async (): Promise<DiagnosticResult> => {
  ...
  try {
    ...
    if (missingVars.length > 0) {
      result.warnings.push(`Missing Firebase environment variables: ${missingVars.join(', ')}`);
      result.recommendations.push('Configure Firebase project at https://console.firebase.google.com/');
      result.recommendations.push('Update .env.local with your Firebase configuration values');
    }
    ...
    if (!auth || !db) {
      result.warnings.push('Firebase services not available - running in demo mode');
    ...
    if (auth) {
      ...
      if (!currentUser) {
        result.recommendations.push('Sign in with garetharjohns@gmail.com to test admin functionality');
      }
    }
    ...
    if (db && auth?.currentUser) {
      try {
        ...
        if (!userSnap.exists()) {
          ...
          result.recommendations.push('User profile will be created on next login');
        }
      } catch (error: any) {
        ...
        if (error.code === 'permission-denied') {
          result.recommendations.push('Deploy Firestore rules: firebase deploy --only firestore:rules');
        }
      }
    ...
    result.routes = {
      ...
      admin: result.authentication.isAuthenticated && result.authentication.isAdmin,
      universe: result.authentication.isAuthenticated
    };

    if (!result.routes.admin && result.authentication.isAuthenticated) {
      result.recommendations.push('Login with garetharjohns@gmail.com to access admin panel');
    }
...
export const logDiagnosticResult = (result: DiagnosticResult) => {
  ...
  console.log('Profile Exists:', result.firestore.userProfileExists ? '✅' : '❌');
