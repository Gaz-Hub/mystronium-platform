...
export class ComprehensiveDiagnostic {
  ...
  async runDiagnostic(): Promise<ComprehensiveDiagnosticResult> {
    ...
    try {
      ...
      this.results.errors.push(`Diagnostic failed: ${error instanceof Error ? error.message : String(error)}`);
      return this.results;
    }
  }
  ...
  private async checkEnvironmentVariables() {
    ...
    Object.entries(requiredVars).forEach(([key, value]) => {
      if (key.startsWith('VITE_FIREBASE_') || key === 'VITE_RECAPTCHA_SITE_KEY') {
  ...
  private async checkFirebaseConfiguration() {
    ...
    this.results.firebase.configuration = {
      apiKey: !!import.meta.env.VITE_FIREBASE_API_KEY && !import.meta.env.VITE_FIREBASE_API_KEY.includes('your_'),
      ...
      measurementId: !!import.meta.env.VITE_FIREBASE_MEASUREMENT_ID && !import.meta.env.VITE_FIREBASE_MEASUREMENT_ID.includes('your_'),
      databaseURL: !!import.meta.env.VITE_FIREBASE_DATABASE_URL && !import.meta.env.VITE_FIREBASE_DATABASE_URL.includes('your_'),
      recaptchaSiteKey: !!import.meta.env.VITE_RECAPTCHA_SITE_KEY && !import.meta.env.VITE_RECAPTCHA_SITE_KEY.includes('your_'),
      configured: false
    };
    ...
    // Check services
    try {
      const { auth, db, storage, realtimeDb, appCheck } = await import('../firebase');
      ...
      if (auth?.currentUser) {
        this.results.firebase.authentication.adminAccess = 
          auth.currentUser.email === this.results.firebase.authentication.adminUser;
      }
      ...
      if (db) {
        try {
          ...
          if (auth?.currentUser) {
            ...
            this.results.firebase.firestore.adminUserExists = userSnap.exists() && userSnap.data()?.admin === true;
          }
        } catch (error: any) {
          if (error.code === 'permission-denied') {
            this.results.firebase.firestore.rulesDeployed = true;
            this.results.warnings.push('Firestore access denied - rules are deployed');
          } else {
      ...
      this.results.firebase.appCheck.recaptchaV3 = !!import.meta.env.VITE_RECAPTCHA_SITE_KEY;
      this.results.firebase.appCheck.enforced = this.results.firebase.appCheck.enabled && this.results.firebase.appCheck.recaptchaV3;

    } catch (error) {
      this.results.errors.push(`Firebase configuration check failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private async checkStripeConfiguration() {
    ...
    this.results.stripe.configuration = {
      publishableKey: !!import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY && !import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY.includes('your_'),
      secretKey: !!import.meta.env.VITE_STRIPE_SECRET_KEY && !import.meta.env.VITE_STRIPE_SECRET_KEY.includes('your_'),
      webhookSecret: !!import.meta.env.VITE_STRIPE_WEBHOOK_SECRET && !import.meta.env.VITE_STRIPE_WEBHOOK_SECRET.includes('your_'),
      configured: false
    };
    ...
    try {
      ...
      this.results.stripe.webhook.signatureValidation = this.results.stripe.configuration.webhookSecret;
      ...
      this.results.warnings.push(`Webhook endpoint not accessible: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    try {
      ...
      this.results.stripe.integration.subscriptionPlans = stripeService.getSubscriptionPlans().length > 0;
      this.results.stripe.integration.checkoutFlow = this.results.stripe.configuration.configured;
    } catch (error) {
      this.results.warnings.push(`Stripe integration check failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private generateRecommendations() {
    ...
    if (!this.results.firebase.configuration.configured) {
      this.results.criticalIssues.push('Firebase not configured - platform will not function');
    }
    
    if (!this.results.firebase.appCheck.enforced) {
      this.results.criticalIssues.push('App Check not enforced - security risk');
    }
    
    if (!this.results.stripe.webhook.accessible) {
      this.results.criticalIssues.push('Stripe webhook not accessible - billing will not work');
    }

    // Recommendations
    if (this.results.environment.missing.length > 0) {
      this.results.recommendations.push('Configure missing environment variables in Netlify dashboard');
    }
    
    if (!this.results.firebase.configuration.configured) {
      this.results.recommendations.push('Set up Firebase project and configure environment variables');
    }
    
    if (!this.results.firebase.appCheck.enforced) {
      this.results.recommendations.push('Enable App Check with reCAPTCHA v3 in Firebase Console');
    }
    
    if (!this.results.stripe.configuration.configured) {
      this.results.recommendations.push('Configure Stripe API keys and webhook secret');
    }
    
    if (!this.results.stripe.webhook.accessible) {
      this.results.recommendations.push('Deploy to Netlify and verify webhook endpoint');
    }
  ...
  private logResults() {
    ...
    console.log('Firebase:', this.results.environment.variables.firebase.length);
    console.log('Stripe:', this.results.environment.variables.stripe.length);
    console.groupEnd();
    
    console.group('🔥 Firebase');
    console.log('Configured:', this.results.firebase.configuration.configured ? '✅' : '❌');
    console.log('Services:', Object.values(this.results.firebase.services).filter(Boolean).length + '/5');
    console.log('App Check:', this.results.firebase.appCheck.enforced ? '✅' : '❌');
    console.log('Admin Access:', this.results.firebase.authentication.adminAccess ? '✅' : '❌');
    console.groupEnd();
    
    console.group('💳 Stripe');
    console.log('Configured:', this.results.stripe.configuration.configured ? '✅' : '❌');
    console.log('Webhook Accessible:', this.results.stripe.webhook.accessible ? '✅' : '❌');
    console.log('Integration:', Object.values(this.results.stripe.integration).filter(Boolean).length + '/3');
    console.groupEnd();
    
    console.group('🔒 Security');
    console.log('App Check:', this.results.security.appCheck ? '✅' : '❌');
    console.log('Firestore Rules:', this.results.security.firestoreRules ? '✅' : '❌');
    console.log('Webhook Signature:', this.results.security.webhookSignature ? '✅' : '❌');
    console.groupEnd();
  ...
  getSummary(): string {
    ...
    const status = criticalCount > 0 ? '🚨 CRITICAL' : 
                   errorCount > 0 ? '❌ ERRORS' : 
                   warningCount > 0 ? '⚠️ WARNINGS' : '✅ HEALTHY';
...
export const runComprehensiveDiagnostic = () => comprehensiveDiagnostic.runDiagnostic(); 